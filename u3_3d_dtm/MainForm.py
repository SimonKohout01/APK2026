...
class Ui_MainWindow(object):
    ...
    def settingsClick(self):
        self.settings_dialog.setWindowTitle("Countour parameters")
        self.ui_settings.spinBox.setValue(self.z_min)
        self.ui_settings.spinBox_2.setValue(self.z_max)
        self.ui_settings.spinBox_3.setValue(self.dz)

        if self.settings_dialog.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            self.z_min = self.ui_settings.spinBox.value()
            self.z_max = self.ui_settings.spinBox_2.value()
            self.dz = self.ui_settings.spinBox_3.value()

    def clearAllClick(self):
        #User defined slot, clear all data including points
        self.Canvas.clearAll()   
        
    def clearResultsClick(self):
        #User defined slot, clear only generated results
    ...
    def createContourLinesClick(self):
        ...
        #Create contour lines
        a = Algorithms()
        contours = a.createContourLines(DT,self.z_min,self.z_max, self.dz)

        #Set results
        self.Canvas.setContours(contours, self.dz)
    ...
    def analyzeSlopeClick(self):
        ...
        #Analyze DTM (both slope and aspect)
        a = Algorithms()
        triangles = a.analyzeDTM(DT)
        
        #Set results
        self.Canvas.setSlope(triangles)
        
        #Toggle checkboxes and views (switch on Slope, switch off Aspect)
    ...
    def analyzeExpositionClick(self):
        ...
        #Toggle checkboxes and views (switch on Aspect, switch off Slope)
    ...
    def viewSlopeClick(self):
        #If slope is turned on, turn off aspect
        if self.actionSlope.isChecked():
            self.actionExposition.setChecked(False)
            self.Canvas.setViewAspect(False)
    ...
    def viewAspectClick(self):
        #If aspect is turned on, turn off slope
        if self.actionExposition.isChecked():
            self.actionSlope.setChecked(False)
            self.Canvas.setViewSlope(False)
    ...
    #AI assistance was utilized here; for a long time, we were unsuccessful and the program kept crashing, 
    #and we didn't know how to make it function correctly. With the help of AI, we finally succeeded
    def openFileClick(self):
        ...
        #Initialize lists for raw coordinates
        xs, ys, zs = [], [], []
        
        #Read file line by line
        with open(file_path, "r") as file:
            for line in file:
                #Remove whitespaces and split by space
                data = line.strip().split()
                
                #Verify if line has at least X, Y, Z coordinates
                if len(data) >= 3:
                    try:
                        # Extract coordinates (replace comma with dot if necessary)
                        xs.append(float(data[0].replace(',', '.')))
                        ys.append(float(data[1].replace(',', '.')))
                        zs.append(float(data[2].replace(',', '.')))
                    except ValueError:
        ...
        #Get bounding box (min and max values)
        min_x, max_x = min(xs), max(xs)
        min_y, max_y = min(ys), max(ys)
        
        #Get canvas dimensions (with small 20px padding)
        width = self.Canvas.width() - 40
        height = self.Canvas.height() - 40
        
        #Compute scale factors
        scale_x = width / (max_x - min_x) if (max_x - min_x) != 0 else 1
        scale_y = height / (max_y - min_y) if (max_y - min_y) != 0 else 1
        scale_factor = min(scale_x, scale_y)

        #Transform and scale coordinates
        points = []
        for x, y, z in zip(xs, ys, zs):
            #Scale X and shift by 20px
            x_scaled = (x - min_x) * scale_factor + 20
            
            #Scale Y, INVERT IT (cartographic correction) and shift by 20px
            y_scaled = (max_y - y) * scale_factor + 20
            
            #Create QPoint3DF (Keep original Z for contour line calculation)
