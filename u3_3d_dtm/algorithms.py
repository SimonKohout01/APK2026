...
class Algorithms:
    ...
    def getAspect(self, p1:QPoint3DF, p2:QPoint3DF, p3:QPoint3DF):
        ...
        raw_aspect = atan2(ny, nx)
        
        if raw_aspect < 0:
            raw_aspect += 2*pi #Convert to 0-2pi interval
       
        aspect = (2 * pi + pi / 2 - raw_aspect) % (2 * pi) #Final conversion to geographical aspect #Qt has inverted y-axis, so aspect is rotated by 90 degrees and inverted
        return aspect
    
    
    #def analyzeDTMaspect(self, DT):
       #triangles = []
        ...
        #for i in range(0, len(DT), 3):
            ...
            #triangle = Triangle(p1, p2, p3)
            #triangle.setSlope(slope)
            #triangle.setAspect(aspect)
            
            #triangles.append(triangle)
        #return triangles


    def analyzeDTM(self, DT):
        #Create list of triangles
        triangles = []
